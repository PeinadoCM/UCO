    ...
    for (size_t i = 1; i < U.size(); ++i)
    {
        ...
        for(size_t j = 0; j < U.size(); ++j){
            for(size_t k=0; k < U.size(); k++){
                auto edge=g.edge(U[j],U[k]);
                if(edge == nullptr){
                    edge=g.edge(U[k],U[j]);
                }
                if(edge != nullptr && A[k] == false && A[j] == true && edge->item() < C[k]){
                    C[k]=edge->item();
                    V[k]=edge->other(U[k])->label();
                }
            }
        }
        //
        ...
        for(size_t j=0; U.size() > j; j++){
            if(A[j] == false && C[j] < least_cost){
                least_cost=C[j];
                u=j;
            }
        }
        ...
        auto edge=g.edge(U[V[u]],U[u]);
        if(edge == nullptr){
            edge=g.edge(U[u],U[V[u]]);
        }
        mst.push_back(edge);
        A[u]=true;
