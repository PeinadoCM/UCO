...
template <class T>
float kruskal_algorithm(Graph<T, float> &g, std::vector<typename Graph<T, float>::EdgeRef> &mst) noexcept(false)
{
    ...
    auto cmp = [](const typename Graph<T, float>::EdgeRef& a, const typename Graph<T, float>::EdgeRef& b) {
    ...
    for(auto edge=edges.begin(); edge != edges.end();edge++){
        int u=sets.find((*edge)->first()->label());
        int v=sets.find((*edge)->second()->label());
