                ...
                int max_acc)
{
    ...
    while (i < log.size() && log[i].time < System().time())
    {
        ...
        else
        {
            ...
            if(counters.has(log[i].ip)){
                auto pos=counters.find(log[i].ip);
                if(counters.entry(pos).is_valid()){
                    size_t aux=counters.entry(pos).value();
                    aux++;
                    counters.entry(pos).set(log[i].ip, aux);
                    if(static_cast<int>(aux)>=max_acc){
    ...
    while (j < log.size() && ((log[j].time + 60) < System().time()))
    {
        if (!(log[j].ip == IP(0, 0, 0, 0)))
        {
            ...
            //    well the work.
            if(counters.has(log[j].ip)){
                auto pos=counters.find(log[j].ip);
                if(counters.entry(pos).is_valid()){
                    size_t aux=counters.entry(pos).value();
                    if(aux>0){
                        aux--;
                        counters.entry(pos).set(log[j].ip, aux);
                    }
                }
            }
            //
