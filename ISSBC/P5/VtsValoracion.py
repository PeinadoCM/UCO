...
class VtsValoracion(QWidget):
    ...
    def actualizarDatosPorDominio(self):
        ...
        onto = get_ontology(archivo_owl).load()
    ...
    def actualizarDescripcion(self, current):
        if current:
            ...
            for criterio in criterios.instances():
                if criterio.valoracion[0].name == self.comboboxValoracion.currentText() and seleccion == criterio.nombre[0]:
                    desc = (
                        f"Nombre:\t\t{criterio.nombre[0]}\n"
                        f"Comparación:\t\t{criterio.tipoComparacion[0]}\n"
                        f"Tipo de resultado:\t{criterio.tipoResultado[0]}\n"
                        f"Puntuación:\t\t{criterio.puntuacion[0]}\n"
                        f"Valor:\t\t{criterio.valor[0]}\n"
                        f"Terminal:\t\t{criterio.obligatoria[0]}"
                    )
